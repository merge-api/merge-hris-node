...
export enum PayGroupApiApiKeys {
    tokenAuth,
}

export class PayGroupApi {
    ...
    public setApiKey(key: PayGroupApiApiKeys, value: string) {
        (this.authentications as any)[PayGroupApiApiKeys[key]].apiKey = value;
    }
     ...
     */
    public async payGroupList (x_account_token: string, created_after?: Date, created_before?: Date, cursor?: string, include_remote_data?: boolean, modified_after?: Date, modified_before?: Date, page_size?: number, remote_id?: string, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: PaginatedPayGroupList;  }> {
        ...
        if (x_account_token === null || x_account_token === undefined) {
            throw new Error('Required parameter x_account_token was null or undefined when calling payGroupList.');
        }
        ...
        return interceptorPromise.then(() => {
            ...
            return new Promise<{ response: http.IncomingMessage; body: PaginatedPayGroupList;  }>((resolve, reject) => {
                localVarRequest(localVarRequestOptions, (error, response, body) => {
                    if (error) {
                        reject(error);
                    } else {
                        body = ObjectSerializer.deserialize(body, "PaginatedPayGroupList");
     ...
     * @param include_remote_data Whether to include the original data Merge fetched from the third-party to produce these models.
     */
    public async payGroupRetrieve (x_account_token: string, id: string, include_remote_data?: boolean, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: PayGroup;  }> {
        ...
        // verify required parameter 'x_account_token' is not null or undefined
        if (x_account_token === null || x_account_token === undefined) {
            throw new Error('Required parameter x_account_token was null or undefined when calling payGroupRetrieve.');
        }
        ...
        return interceptorPromise.then(() => {
            ...
            return new Promise<{ response: http.IncomingMessage; body: PayGroup;  }>((resolve, reject) => {
                localVarRequest(localVarRequestOptions, (error, response, body) => {
                    if (error) {
                        reject(error);
                    } else {
                        body = ObjectSerializer.deserialize(body, "PayGroup");
                        if (response.statusCode && response.statusCode >= 200 && response.statusCode <= 299) {
                            resolve({ response: response, body: body });
                        } else {
                            reject(new HttpError(response, body, response.statusCode));
