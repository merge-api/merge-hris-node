...
export enum PersistentDeductionsApiApiKeys {
    tokenAuth,
}

export class PersistentDeductionsApi {
    ...
    public setApiKey(key: PersistentDeductionsApiApiKeys, value: string) {
        (this.authentications as any)[PersistentDeductionsApiApiKeys[key]].apiKey = value;
    }
    ...
    /**
     * Creates a `PersistentDeduction` object with the given values.
     ...
     * @param persistentDeductionRequest 
     */
    public async persistentDeductionsCreate (xAccountToken: string, runAsync?: boolean, persistentDeductionRequest?: PersistentDeductionRequest, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: PersistentDeduction;  }> {
        ...
        let localVarRequestOptions: localVarRequest.Options = {
            ...
            json: true,
            body: ObjectSerializer.serialize(persistentDeductionRequest, "PersistentDeductionRequest")
        };
        ...
        return interceptorPromise.then(() => {
            ...
            return new Promise<{ response: http.IncomingMessage; body: PersistentDeduction;  }>((resolve, reject) => {
                localVarRequest(localVarRequestOptions, (error, response, body) => {
                    if (error) {
                        reject(error);
                    } else {
                        body = ObjectSerializer.deserialize(body, "PersistentDeduction");
    ...
    /**
     * Returns a list of `PersistentDeduction` objects.
     * @param xAccountToken Token identifying the end user.
     ...
     * @param employeeId If provided, will only return deductions for this employee.
    ...
    public async persistentDeductionsList (xAccountToken: string, createdAfter?: Date, createdBefore?: Date, cursor?: string, employeeId?: string, includeRemoteData?: boolean, modifiedAfter?: Date, modifiedBefore?: Date, pageSize?: number, remoteId?: string, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: PaginatedPersistentDeductionList;  }> {
        ...
        return interceptorPromise.then(() => {
            ...
            return new Promise<{ response: http.IncomingMessage; body: PaginatedPersistentDeductionList;  }>((resolve, reject) => {
                localVarRequest(localVarRequestOptions, (error, response, body) => {
                    if (error) {
                        reject(error);
                    } else {
                        body = ObjectSerializer.deserialize(body, "PaginatedPersistentDeductionList");
                        if (response.statusCode && response.statusCode >= 200 && response.statusCode <= 299) {
                            resolve({ response: response, body: body });
                        } else {
                            reject(new HttpError(response, body, response.statusCode));
    ...
    /**
     * Returns a `PersistentDeduction` object with the given `id`.
     * @param xAccountToken Token identifying the end user.
     * @param id 
     * @param includeRemoteData Whether to include the original data Merge fetched from the third-party to produce these models.
     */
    public async persistentDeductionsRetrieve (xAccountToken: string, id: string, includeRemoteData?: boolean, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: PersistentDeduction;  }> {
