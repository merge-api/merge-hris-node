...
export class PayrollRunsApi {
    ...
    public async payrollRunsList (x_account_token: string, created_after?: Date, created_before?: Date, cursor?: string, ended_after?: Date, ended_before?: Date, include_remote_data?: boolean, modified_after?: Date, modified_before?: Date, page_size?: number, remote_id?: string, run_type?: '' | 'CORRECTION' | 'null' | 'OFF_CYCLE' | 'REGULAR' | 'SIGN_ON_BONUS' | 'TERMINATION', started_after?: Date, started_before?: Date, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: PaginatedPayrollRunList;  }> {
        ...
        if (run_type !== undefined) {
            localVarQueryParameters['run_type'] = ObjectSerializer.serialize(run_type, "'' | 'CORRECTION' | 'null' | 'OFF_CYCLE' | 'REGULAR' | 'SIGN_ON_BONUS' | 'TERMINATION'");
        }
